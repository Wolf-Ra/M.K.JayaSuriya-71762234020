...
f=open('Input\Input\Milestone1.json')
data = json.load(f)
...
for quant in range(len(list_wafer)):
    ...
    if machine_time['M1']>machine_time['M3']:
        mach_status['M3'] = 'Free'
    else:
        machine_time['M1'] = machine_time['M3']
        mach_status['M3'] = 'Free'
    
    if machine_time['M1']<machine_time['M3']:
        mach_status['M1'] = 'Free'
    else:
        machine_time['M3'] = machine_time['M1']
        mach_status['M1'] = 'Free'

    if machine_time['M2']<machine_time['M3']:
        mach_status['M2'] = 'Free'
    else:
        machine_time['M3'] = machine_time['M2']
        mach_status['M2'] = 'Free'

    if machine_time['M2']>machine_time['M3']:
        mach_status['M3'] = 'Free'
    else:
        machine_time['M2'] = machine_time['M3']
        mach_status['M3'] = 'Free'
    ...
    if 'S2' not in wafers_process[list_wafer[len(list_wafer)-(quant+2)]] and mach_status['M3'] != 'Busy':
        wafers_process[list_wafer[len(list_wafer)-(quant+2)]].append('S2')
        init_time = machine_time['M3']
        machine_time['M3'] = init_time + wafer['W1']['process_time']['S2']
        mach_status['M3'] = 'Busy'
        schedule['wafer_id'] = list_wafer[len(list_wafer)-(quant+2)]
        schedule['step'] = 'S2'
        schedule['machine'] = 'M3'
        schedule['start_time'] = init_time
        schedule['end_time'] = machine_time['M3']
        allocation.append(schedule)
        schedule = dict()
        init_time = machine_time['M3']
    else:
...
with open("Milestone1Solution.json", "w") as outfile: 
